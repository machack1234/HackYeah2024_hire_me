...
const generateLearningPlan = async (context: string) => {
	try {
		...
		const learningPlan = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/generate-course`, {
			context,
		});

		console.log(learningPlan.data.answer);
	...
	const result = await streamText({
		...
		tools: {
			generateLearningPlan: {
				...
				parameters: z.object({
					context: z.string().describe('The generated learning plan in JSON format'),
				}),
				execute: async ({ context }) => {
					...
					console.log('Generated Learning Plan JSON:', JSON.stringify(learningPlan, null, 2));
					return learningPlan;
				},
			},
		},
